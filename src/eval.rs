...
impl BasicEvaluator {
    pub(crate) fn new() -> Self {
        Self {
            queen_liberty_penalty: 808.20465f32,
            gates_factor: 1.908362f32,
            queen_spawn_factor: 67.84935f32,
            unplayed_bug_factor: -0.224473f32,
            pillbug_defense_bonus: 296.49698f32,
            ant_game_factor: -0.460803f32,
            queen_score: 254.94263f32,
            ant_score: 232.27896f32,
            beetle_score: 459.46613f32,
            grasshopper_score: 281.7737f32,
            spider_score: 47.66202f32,
            mosquito_score: 1.55345f32,
            ladybug_score: 71.03928f32,
            pillbug_score: 187.15604f32,
            mosquito_incremental_score: 0.171273f32,
            stacked_bug_factor: -0.31868f32,
            queen_movable_penalty_factor: 0.4325358f32,
            opponent_queen_liberty_penalty_factor: 1.8123524f32,
            trap_queen_penalty: 381.5815f32,
            placeable_pillbug_defense_bonus: 61.132866f32,
            pinnable_beetle_factor: 0.56840235f32,
            //new factors
            mosquito_ant_factor: 13.120331f32,
            mobility_factor: 0.43263993f32,
            compactness_factor: -0.75910336f32,
            pocket_factor: -0.048804115f32,
            beetle_attack_factor: -4.8218513f32,
            beetle_on_enemy_queen_factor: 1377.3895f32,
            beetle_on_enemy_pillbug_factor: 421.05048f32,
            direct_queen_drop_factor: 7.7612314f32,
        }
    }
...
fn triangles_from_hex(board: &Board, hex: Hex) -> u8 {
    //check if in consecutive adjacent hexes there are two pieces of the same color
    let mut count = 0;
    let mut prev = hex;
    let color = board.node(hex).color();
...
fn four_pieces_from_hex(board: &Board, hex: Hex) -> u8 {
    //check if in consecutive adjacent hexes there are three pieces of the same color
    //note that it will be counted two times, given the hexagonal form
    let mut count = 0;
    let mut prev1 = hex;
    let mut prev2 = hex;
    let color = board.node(hex).color();
...
fn pocket_from_low_angle_hex(board: &Board, hex: Hex) -> bool {
    //a pocket has this form:
    // - three pieces of the same color in a row, the major basis: low angle 1, mid, low angle 1
    // - two pieces of the same color in a row, the minor basis -> this with the mid hex forms a triangle
    //note that the mid hex actually forms three triangles, the other two are with the low angle hexes and ne hex of the minor basis
    //note finally that if mid forms at least three triangles it has to be a pocket, it's easy visible graphically
